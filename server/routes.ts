...
import { z } from "zod";
import { db, users, partners, projects, notifications, tickets, payments, paymentStages, portfolio, referrals, projectMessages, projectFiles, projectTimeline, ticketResponses, paymentMethods, invoices, transactions, budgetNegotiations, twilioConfig } from "./db";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/payment-stages/:id/confirm-payment", authenticateToken, async (req: AuthRequest, res) => {
    try {
      const stageId = parseInt(req.params.id);
      const { paymentMethod, proofFileInfo } = req.body;
      ...
      const updated = await storage.updatePaymentStage(stageId, {
        paymentMethod,
        proofFileUrl: proofFileInfo ? `comprobante_${stageId}_${Date.now()}.${proofFileInfo.fileType?.split('/')[1] || 'jpg'}` : null,
      ...
      for (const admin of adminUsers) {
        await storage.createNotification({
          ...
          message: `El cliente ${client.fullName} confirmó el pago de la etapa "${stage[0].stageName}" mediante ${paymentMethod}. Verificar comprobante.`,
          type: "info",
        });
      }
      ...
      try {
        // Notificar al admin por email con el comprobante
        await sendPaymentProofNotificationToAdmin(
          ...
          stage[0].amount,
          paymentMethod,
          proofFileInfo ? `Archivo adjunto: ${proofFileInfo.fileName} (${(proofFileInfo.fileSize / 1024 / 1024).toFixed(2)} MB)` : null
        );
