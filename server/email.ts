...
export const sendPaymentProofNotificationToAdmin = async (
    ...
    <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="padding: 30px 0;">
        ...
        <div style="background: #fffbeb; border: 1px solid #fbbf24; border-radius: 8px; padding: 20px; margin: 20px 0;">
          <h3 style="margin: 0 0 10px 0; color: #d97706;">📎 Comprobante adjunto:</h3>
          <p style="margin: 5px 0;">Se ha adjuntado un archivo de comprobante.</p>
          <p style="margin: 5px 0;"><strong>URL:</strong> <a href="${proofFileUrl}" style="color: #d97706;">${proofFileUrl}</a></p>
        </div>
        ` : ''}

        <div style="text-align: center; margin: 30px 0;">
          <a href="https://softwarepar.lat/admin/projects" style="background: #059669; color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; display: inline-block;">Ver en Dashboard</a>
        </div>

        <p><strong>Acción requerida:</strong> Por favor revisa y verifica el comprobante para confirmar el pago.</p>
  ...
  await sendEmail({
    to: "softwarepar.lat@gmail.com",
    subject: `Comprobante de pago: ${projectName} - ${stageName} ($${amount})`,
    html,
  });
};
