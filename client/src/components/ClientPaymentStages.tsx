...
const PaymentOptionsModal = ({ isOpen, onClose, amount, stageName, stageId, projectName }) => {
  ...
  const handleSendProof = async () => {
    ...
    try {
      ...
      // Si hay archivo seleccionado, preparar información
      if (selectedFile) {
        proofFileInfo = {
      ...
      // Enviar datos del pago al servidor para notificaciones por email
      const response = await apiRequest('POST', `/api/payment-stages/${stageId}/confirm-payment`, {
        paymentMethod: selectedMethodData.name,
        proofFileInfo: proofFileInfo
      });</old_str>
      ...
      if (response.ok) {
        ...
        const whatsappMessage = encodeURIComponent(
          ...
          `${selectedFile ? '📎 He adjuntado el comprobante en el sistema.\n\n' : ''}` +
        ...
        alert(`Error al enviar el comprobante: ${errorData.message || 'Error desconocido'}`);
      }</old_str>
    } catch (error) {
      console.error('Error enviando comprobante:', error);
      alert('Error al enviar el comprobante. Verifique su conexión e inténtalo de nuevo.');
    }
  };
        ...
        {!showPaymentDetails ? (
          ...
          <>
            ...
            <button 
              ...
              className="w-full bg-green-500 text-white py-3 px-4 rounded-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 font-medium text-lg"
            >
